...
class btMultiBodyLinkGhoster : public btGhostObject
{
	//protected:
public:
	btMultiBody* m_multiBody;
	...
	virtual ~btMultiBodyLinkGhoster()
	{

	}
	btMultiBodyLinkGhoster(btMultiBody* multiBody, int link)
		: m_multiBody(multiBody),
		  m_link(link)
	{
		m_checkCollideWith = true;
	...
	static btMultiBodyLinkGhoster* upcast(btCollisionObject* colObj)
	{
		if (colObj->getInternalType() & btCollisionObject::CO_FEATHERSTONE_LINK)
			return (btMultiBodyLinkGhoster*)colObj;
		return 0;
	}
	static const btMultiBodyLinkGhoster* upcast(const btCollisionObject* colObj)
	{
		if (colObj->getInternalType() & btCollisionObject::CO_FEATHERSTONE_LINK)
			return (btMultiBodyLinkGhoster*)colObj;
		return 0;
	...
	virtual bool checkCollideWithOverride(const btCollisionObject* co) const
	{
		const btMultiBodyLinkGhoster* other = btMultiBodyLinkGhoster::upcast(co);
		if (!other)
			return true;
		if (other->m_multiBody != this->m_multiBody)
			return true;
...
SIMD_FORCE_INLINE int btMultiBodyLinkGhoster::calculateSerializeBufferSize() const
{
	return sizeof(btMultiBodyLinkColliderData);
}

SIMD_FORCE_INLINE const char* btMultiBodyLinkGhoster::serialize(void* dataBuffer, class btSerializer* serializer) const
{
	btMultiBodyLinkColliderData* dataOut = (btMultiBodyLinkColliderData*)dataBuffer;
	btCollisionObject::serialize(&dataOut->m_colObjData, serializer);
